...
Wh214.SD.1 <- cbind.data.frame(Wh214_A_0$syllable.duration, 
...
Time <- C("Pre", "Post 1M", "Post 2M", "Post 3M")
Condition <- c("A", "B", "C", "D", "E", "F", "G")

SD.Summary <- Wh214.SD %>% # name of data frame read into R
  group_by(Timepoint,Syllable) %>% # grouping variables
  summarise(Mean = mean(value), # calculates mean of each group
            sd = sd(value), # calculates Standard deviation
            N = n(), # calculates sample size per group
            se = sd/sqrt(N)) # calculates standard error

PD <- position_dodge(.1) # moves points .05 to left and right
  
ggplot(data = SD.Summary, aes(y = SD.Summary$Mean, 
                              x = SD.Summary$Timepoint)) +
  geom_point(aes(y = SD.Summary$Mean, fill = SD.Summary$Timepoint), 
             stat = "summary",fun.y = "mean", position = PD) +
  geom_errorbar(aes(ymin = SD.Summary$Mean - SD.Summary$se,
                    ymax = SD.Summary$Mean + SD.Summary$se),
                width = .1, position = PD) +
  scale_y_continuous(name = "Syllable Duration (ms)")+
  ggtitle("White 214") +
...
# THIS GRAPH WORKS Kindof


Wh214_SD <- ggplot(data = birdfile, aes(x = birdfile$Timepoint, 
                            y = birdfile$syllable.duration)) +
  geom_bar(aes(y = Wh214.SD$value, fill = Wh214.SD$Timepoint), 
