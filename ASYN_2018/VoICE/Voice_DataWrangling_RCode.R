...
#  ZScore <- (x - mean(x, ma.rm = TRUE))/Std.Dev
# )
...
ZScore <- function(x) {
  zscore = ((x - mean(x, ma.rm = TRUE))/sd(x, na.rm=TRUE))
}
...
##- Create zscore function to normalize values 
##- by baseline timepoint for each Bird.Syll

ZScore <- function(data, condition_name, variable_name){
  
  data<-as.data.frame(data)
  lev<-unique(data[,condition_name])
  ...
  for(k in seq_along(lev_vars)){
    ...
    for(i in 1: length(lev) ){ 
      ...
      b <- a[a[,condition_name] == lev[i],]
      
      ZScoreMean <- (b$mean - mean(b$mean))/sd(b$mean)
      
      ZScoreCV <- (b$CV - mean(b$CV))/sd(b$CV)
      
      newmatrix <- cbind.data.frame(b,ZScoreMean,ZScoreCV)
...
df <- ZScore(data = df, condition_name = "Condition", variable_name = "Variable")

df <- unlist(df, recursive = FALSE) # concatonates a list of lists into one list
...
df <- gather(data = df, key = Score, value = Values, c(3:4,9, 14:17,19,20))
...
ConditionSummary <- as.data.frame(summaryBy(mean + CV + NormMean + NormCV + 
                                            modulationindex.mean + modulationindex.CV + ZScoreMean +
                                            ZScoreCV ~ Condition + Variable + Timepoint, 
                                            data = df,
                                            FUN = c(mean, CV, length, sd, std.error)))

ConditionSummary <- within(ConditionSummary, Variable.Time <- 
                             str_c(ConditionSummary$Variable, ConditionSummary$Timepoint,sep = "."))

# ConditionSummary <- ConditionSummary[complete.cases(ConditionSummary),]

ConditionSummary <- ConditionSummary[ ,c(1:11,28:44)]
